...
uint8_t find_closest_color(uint8_t r, uint8_t g, uint8_t b, uint8_t *palette, int palette_size)
{
...
int main(int argc, char *argv[])
{
	...
	if (!gif)
	{
		...
		if (in_path != filename_in) free(in_path);
		return 1;
	}
	...
	if (!resized_gif)
	{
		...
		gd_close_gif(gif);
		if (out_path && out_path != filename_out) free(out_path);
		if (in_path != filename_in) free(in_path);
		return 1;
	}
	...
	{
		printf("\rFrame: %i", frame_index);
		fflush(stdout);
		indexed_frame = gif->frame;
		rgba_buffer = malloc(gif->width * gif->height * 4);

		// rgb -> rgba
		for (int j = 0; j < gif->width * gif->height; j++)
		{
			uint8_t pallette_index = indexed_frame[j];
			uint8_t *color = &gif->palette->colors[pallette_index * 3];
			rgba_buffer[j * 4] = color[0];	   // R
			rgba_buffer[j * 4 + 1] = color[1]; // G
			rgba_buffer[j * 4 + 2] = color[2]; // B
			rgba_buffer[j * 4 + 3] = 255;	   // Alpha
		}
		...
		for (int j = 0; j < new_width * new_height; j++)
		{
			...
			uint8_t palette_index = find_closest_color(r, g, b, gif->palette->colors, gif->palette->size);
			resized_gif->frame[j] = palette_index;
		}

		ge_add_frame(resized_gif, 10);
